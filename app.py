...
with gr.Blocks(title="College Chatbot") as demo:
    ...
    with gr.Row():
        college_dropdown = gr.Dropdown(
            label="Select College", choices=["pict", "vit"], value=None
        )

    chatbot = gr.Chatbot(label="Chat")
    msg = gr.Textbox(
        placeholder="Ask something about the selected college...", show_label=False
    )

    clear = gr.Button("Clear Chat")

    msg.submit(put_message_in_chatbot, [msg, chatbot], [msg, chatbot]).then(
        qa, inputs=[msg, chatbot, college_dropdown], outputs=[chatbot]
    )
