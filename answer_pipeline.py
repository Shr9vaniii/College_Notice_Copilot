...
class CollegeChatbot:
    ...
    def rerank(self, question, chunks):
        ...
        ranked = sorted(
            ranked_by_semantic,
            key=lambda x: (x[1], x[0].metadata.get("upload_date", 0)),
            reverse=True,
        )
    ...
    def query_rewriting(self, query, history):
        ...
        system_msg = """You are an excellent query_rewriter. You are provided with the user question and the history.
    ...
    def fetch_context_unranked(
        self, question, college_name, k: int = DEFAULT_RETRIEVAL_K
    ):
        t0 = time.time()
        ...
        t2 = time.time()
        chunks = []
        print({"retriever_creation": t1 - t0, "invoke_time": t2 - t1})
