...
def extract_text_from_pdf(pdf_path, college_id, client=client):
    ...
    embeddings = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")
    vectorstore1 = Chroma(
        client=client, collection_name=college_collection, embedding_function=embeddings
    )
    exists = vectorstore1._collection.get(where={"source": pdf_path}, limit=1)
...
def get_text_processed(text):
    system_msg = """You are an excellent text_processor. You will be provided with some unprocessed text which can be text extracted from tables,images, etc. Process the text so it can be easily chunked into meaningful data to be stored in vector database.
...
async def process_pdfs(s3_key, college_id):
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
            ...
            await asyncio.to_thread(extract_text_from_pdf, temp_path, college_id)
